...
function updatePath() {
    ...
    fullPathElement.innerHTML = systemSerial + ".txt";
}
...
// function to publish xmlDoc nd globalBase64TstPkg
function publishXmlAndTstPkg() {
    console.log('publishXmlAndTstPkg() called!');
    // get globalBase64TstPkg from sessionStorage
...
function getRowContent(cells, itemIndex, retCodeIndex, reasonIndex) {
    ...
    const retCode = cells[retCodeIndex].textContent === 'PASS' ? 1 : 0;
...
function addVerToDeviceInfo(message) {
    ...
    const payload = message.payloadString;
    var payloadReplaced = payload.replace(/<br>/g, '\n');
    var version = payloadReplaced.split('FW Version=')[1].split('\n')[0];
